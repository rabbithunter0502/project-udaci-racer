...
async function handleCreateRace() {
	...
	if (!playerId) {
		alert('please choose your racer!')
		location.reload();
	}
	try {
		...
		const process = await runRace(store.race_id);
	  } catch (err) {
		console.log(err);
	  }
}
...
async function  runRace(raceID) {
	...
	return new Promise(resolve => {
		...
		const raceInterval = setInterval(async () => {
			...
			*/
			if (raceResponse.status !== "finished") {
...
function renderRaceStartView(track, racers) {
	...
	return `
		<header>
			<h1 class="header-text">Race: ${track}</h1>
		</header>
		<main id="two-columns">
...
function resultsView(positions) {
	...
	return `
		<header>
			<h1 class="header-text">Race Results</h1>
		</header>
		<main>
			${raceProgress(positions)}
			<a href="/race" class="button" style="margin: 20px auto;">Start a new race</a>
		</main>
	`
}
...
function raceProgress(positions) {
	...
	return `
		<main>
			<h3 style="margin: 20px auto; text-align: center; font-weight: 700;">Leaderboard</h3>
			<section id="leaderBoard">
				${results}
			</section>
		</main>
